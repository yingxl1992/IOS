...
- (void)saveRecord {
    ...
    NSURL *url;
    if (flag==0) {
        url=[NSURL URLWithString:@"http://localhost:8080/expend/add"];
    } else {
        url=[NSURL URLWithString:@"http://localhost:8080/income/add"];
    }
    ...
    NSString *strId=[[NSString alloc]initWithData:received encoding:NSUTF8StringEncoding];
    if ([strId compare:@"0"]==NSOrderedSame) {
        UIAlertView *alertView=[[UIAlertView alloc]initWithTitle:nil message:@"添加失败，请重试~" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles: nil];
        [alertView show];
    } else {
...
- (void)initRecord {    
    ...
    [record setAddTime:currentDate];
    [record setMoney:0];
    [record setDes:@""];
    [record setType:0];
    [record setAccountType:0];
    NSArray *days=[currentDate componentsSeparatedByString:@"/"];
    [record setYear:[[NSString stringWithFormat:@"%@",[days objectAtIndex:0]] integerValue]];
    [record setMonth:[[NSString stringWithFormat:@"%@",[days objectAtIndex:1]] integerValue]];
    [record setDay:[[NSString stringWithFormat:@"%@",[days objectAtIndex:2]] integerValue]];
}
